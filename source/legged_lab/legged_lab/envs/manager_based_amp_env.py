...
class ManagerBasedAmpEnv(ManagerBasedAnimationEnv):
    ...
    def load_managers(self):
        ...
        # --- per-joint action delay buffer for communication-latency domain randomization ---
        self._max_action_delay = self.cfg.max_action_delay_steps
        if self._max_action_delay > 0:
            action_dim = self.action_manager.total_action_dim
            # shape: (num_envs, max_delay+1, action_dim); ring buffer per joint
            self._action_delay_buf = torch.zeros(
                self.num_envs, self._max_action_delay + 1, action_dim, device=self.device
            )
            # per-joint delay: shape (num_envs, action_dim)
            self._action_delay_steps = torch.zeros(
                self.num_envs, action_dim, dtype=torch.long, device=self.device
            )
            self._action_dim = action_dim
            print(f"[INFO] Per-joint action delay enabled: 0–{self._max_action_delay} steps, "
                  f"prob={self.cfg.action_delay_probability}.")

    def _reset_idx(self, env_ids: torch.Tensor):
        super()._reset_idx(env_ids)
        if self._max_action_delay > 0:
            n = len(env_ids)
            # Per-joint: draw delay from [1, max_delay], shape (n, action_dim)
            delays = torch.randint(
                1, self._max_action_delay + 1, (n, self._action_dim), device=self.device
            )
            # Per-joint probability mask
            mask = torch.rand(n, self._action_dim, device=self.device) < self.cfg.action_delay_probability
            self._action_delay_steps[env_ids] = delays * mask
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        """
        # process actions (apply per-joint communication delay if configured)
        if self._max_action_delay > 0:
            self._action_delay_buf = self._action_delay_buf.roll(1, dims=1)
            self._action_delay_buf[:, 0] = action.to(self.device)
            # gather per-joint: _action_delay_steps is (num_envs, action_dim)
            env_idx = torch.arange(self.num_envs, device=self.device).unsqueeze(1)  # (N, 1)
            joint_idx = torch.arange(self._action_dim, device=self.device).unsqueeze(0)  # (1, D)
            delayed_action = self._action_delay_buf[env_idx, self._action_delay_steps, joint_idx]
