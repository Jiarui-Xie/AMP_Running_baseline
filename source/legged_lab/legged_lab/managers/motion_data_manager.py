...
class MotionDataTerm(ManagerTermBase):
    ...
    def _load_motion_data(self):
        # list the motion data files in the directory recursively
        motion_files = []
        for root, _, files in os.walk(self.cfg.motion_data_dir):
            for f in files:
                if f.endswith('.pkl'):
                    # get relative path and normalize slashes to make dictionary keys work across platforms
                    rel_path = os.path.relpath(os.path.join(root, f), self.cfg.motion_data_dir)
                    motion_files.append(rel_path.replace(os.sep, '/'))
